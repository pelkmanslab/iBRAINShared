function [matCompleteData, strFinalFieldName, matCompleteMetaData, structDataColumnsToUse, structMiscSettings, cellstrDataPaths] = getRawProbModelData2(strRootPath,strSettingsFile,intNumOfDirs, structDataColumnsToUse, structMiscSettings)
% Usage: 
%
...
% 
% % To consider only sites with consistent counts of nuclei, cells and/or
% % cytoplasm, use the following optional command.
% structMiscSettings.requireConsistentObjectAmount = true 
% 
...
%
    ...
    if nargin <= 3
        ...
        if nargin>3 && isempty(structDataColumnsToUse)
            disp(sprintf('%s:  retreiving settings from %s',mfilename,strSettingsFile))            
            [structDataColumnsToUse, structMiscSettings] = initStructDataColumnsToUse(strSettingsFile);
        else
    ...
    % retry...
    cellstrDataPaths = findPlates(strRootPath);
    ...
    if isempty(cellstrDataPaths)
        disp(sprintf('%s:  found %d plates, assuming the path is a plate\n',mfilename,size(cellstrDataPaths,1)))
        cellstrDataPaths = {strRootPath};
    end
    ...
    for iDir = intNumOfDirs
        ...
        for i = 1:length(cellstrNucleiFieldnames)%%% ORIGINAL
            ...
            for iiX = 1:length(structDataColumnsToUse.(strMeasurementBlockName))
                ...
                % get measurement features in cell array
                FeaturesAvailable = false;
                if isfield(PlateDataHandles.Measurements.(strObjectName),strcat(strMeasurementName,'Features'))
                    strMeasurementFeatureName = strcat(strMeasurementName,'Features');
                    cellMeasurementFeatures = PlateDataHandles.Measurements.(strObjectName).(strMeasurementFeatureName);
...
%                       strFinalFieldName = [strFinalFieldName, sprintf('%s_%s_%d_%s',strMeasurementName,strObjectName,intColumnIX,cellMeasurementFeatures{ii})];  % old
                        strFinalFieldName = [strFinalFieldName, sprintf('%s_%s_%d_%s',strMeasurementName,strObjectName,intColumnIX,cellMeasurementFeatures{intColumnIX})]; %[Suggested Fix by TS on 150123]
                    else
                    ...
                    try % [GG} this try catch is trying to resolvethe problem if objectcounts between cells and nuclei are not equal. it does not get activated if getRawProbModelData2 runs on a cluster
                    matPlateData = cellfun(@(x,y) x(y,1), cellCurrentMeasurementColumn(matNonEmptyImages),cellObjectsToInclude(matNonEmptyImages),'UniformOutput',false);
                        ...
                        if ~checkIfIAmWorkingOnACluster
                        reply = input('We found an error in the indexing between parent and chuld objects, you want to try fixing it? Y/N [Y]: ', 's');
                        if ~isempty(reply)
                            countMesObj = cell2mat(cellfun(@(x) numel(x), cellCurrentMeasurementColumn(matNonEmptyImages),'UniformOutput',false));
                            countObjToInc = cell2mat(cellfun(@(x) numel(x), cellObjectsToInclude(matNonEmptyImages),'UniformOutput',false));
                            posToChange = find(countMesObj ~= countObjToInc);
                            tmpMesurments = cellCurrentMeasurementColumn(matNonEmptyImages);
                            tmpObjects = cellObjectsToInclude(matNonEmptyImages);
                            
                            for ix= 1:numel(posToChange)
                                tmpPos = posToChange(ix);
                                changer  = Inf(size(tmpObjects{tmpPos}))./-0;
                                tmpMesurments{tmpPos} = changer;
                            end
                            cellCurrentMeasurementColumn(matNonEmptyImages) = tmpMesurments;
                            matPlateData = cellfun(@(x,y) x(y,1), cellCurrentMeasurementColumn(matNonEmptyImages),cellObjectsToInclude(matNonEmptyImages),'UniformOutput',false);
                        end
                        end
                    end
...
%             size(matCompleteData)
    ...
    %[GG]implementation of site integrity control
    matMinusInfRowIndices = any(matCompleteData== -inf,2);
        if sum(matMinusInfRowIndices) > 0
            fprintf('%s:  removing %d rows with -inf values\n',mfilename,sum(matMinusInfRowIndices))
            matCompleteData(matMinusInfRowIndices,:) = [];
            matCompleteMetaData(matMinusInfRowIndices,:) = [];
        end
...
if fileattrib(fullfile(strRootPath,[strSvmStrMatch,'.mat']))
    strSvmMeasurementName = [strSvmStrMatch,'.mat'];
    return
end
